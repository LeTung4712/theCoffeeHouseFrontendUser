...
export default {
  ...
  components: {
    AuthenticationUser: () =>
      import("@/components/userComponents/Authentication_User"),
  },
  data: () => {
    return {
      oldAddress: "...",
  ...
  created() { // chạy trước khi render
    if (localStorage.getItem("order") == null) { // nếu chưa có order thì itemCount = 0
      this.itemCount = 0;
    } else {
    ...
    if (localStorage.getItem("user") == null) { // nếu chưa đăng nhập thì logged = 0
      this.logged = 0;
    } else {
    ...
    if (localStorage.getItem("oldAddress") != undefined) { // nếu đã có địa chỉ thì lấy địa chỉ
