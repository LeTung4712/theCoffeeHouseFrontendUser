<template>
  <div style="top: -206px">
    <v-row justify="center">
      <v-dialog transition="dialog-top-transition" max-width="26.25rem" content-class="my-custom-dialog">
        <template v-slot:activator="{ on, attrs }">
          <p color="primary" v-bind="attrs" v-on="on" style="margin-left: 17px; width: 124.75px; margin-bottom: 0">Login
          </p>
        </template>
        <template>
          <div class="login_modal">
            ...
            <div class="login_content" :class="{ hidden: loginHidden }">
              ...
              <img class="logo_tch"
                src="https://cdn.haitrieu.com/wp-content/uploads/2022/03/logo-the-coffee-house-chieu-dai.png" alt=""
                style="width: 65%">
              <!--            nhập sdt người dùng -->
              <form @submit.prevent="sendPhoneNumber">
                ...
                <v-btn type="submit" @click="otpActive = !otpActive; loginHidden = !loginHidden" width="100%" block
                  depressed color="#e87800" class="login_btn">Đăng nhập</v-btn>
              </form>
            </div>
                ...
                <div class="cover-input-otp">
                  <div style="display: flex;justify-content: space-around">
                    <div style="display: flex;align-items: center">
                      <input type="tel" class="otp-input" min="0" max="9" maxlength="1" pattern="[0-9]" id="input1"
                        @keyup="inputenter()">
                    </div>
                    <div style="display: flex;align-items: center">
                      <input type="tel" class="otp-input" min="0" max="9" maxlength="1" pattern="[0-9]" id="input2"
                        @keyup="inputenter()">
                    </div>
                    <div style="display: flex;align-items: center">
                      <input type="tel" class="otp-input" min="0" max="9" maxlength="1" pattern="[0-9]" id="input3"
                        @keyup="inputenter()">
                    </div>
                    <div style="display: flex;align-items: center">
                      <input type="tel" class="otp-input" min="0" max="9" maxlength="1" pattern="[0-9]" id="input4"
                        @keyup="inputenter()">
                    </div>
                    <div style="display: flex;align-items: center">
                      <input type="tel" class="otp-input" min="0" max="9" maxlength="1" pattern="[0-9]" id="input5"
                        @keyup="inputenter()">
                    </div>
                    <div style="display: flex;align-items: center">
                      <input type="tel" class="otp-input" min="0" max="9" maxlength="1" pattern="[0-9]" id="input6"
                        @keyup="inputenter()">
                    </div>
                  </div>
                </div>
                <div class="welcome-login" style="margin-bottom: 0.5rem"> Bạn không nhận được mã ?
                  <a class="addborder" style="text-decoration: none;cursor: pointer;margin-left: 0.25rem" href=""> Gửi
                    lại</a>
                </div>
  ...
  methods: {
    ...
    sendPhoneNumber() {
      ...
      axios.post("http://127.0.0.1:8000/api/user/auth/login", {
        mobile_no: this.data.mobile_no //vd http://127.0.0.1:8000/api/user/auth/login?mobile_no=+84123456789
      }).then((response) => {
